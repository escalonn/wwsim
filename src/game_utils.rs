...
/// Find a new capital for `country_id` using BFS-shell expansion from `original_capital_id`.
/// Searches shell by shell (including shells of foreign territory) until a shell contains at
/// least one territory owned by `country_id`, then picks one of those randomly.
...
pub fn find_new_capital(
    ...
    let mut visited = HashSet::new();
    visited.insert(original_capital_id);

    let mut current_shell: Vec<u16> = targets_data[&original_capital_id]
        .iter()
        .copied()
    ...
    loop {
        if current_shell.is_empty() {
            // Entire map exhausted without finding any surviving territory — shouldn't happen.
            return original_capital_id;
        }

        let eligible: Vec<u16> = current_shell.iter()
            .copied()
            .filter(|&id| owners_data.get(&id) == Some(&country_id))
            .collect();

        if !eligible.is_empty() {
            return *eligible.choose(&mut rng).unwrap();
        }

        // Expand to next shell
        let mut next_shell = Vec::new();
        for id in &current_shell {
            for &neighbor in &targets_data[id] {
                if visited.insert(neighbor) {
...
/// Valid means: it is owned by `country_id` AND it is in the nearest BFS shell from
/// `original_capital_id` that contains at least one territory owned by `country_id`.
pub fn validate_new_capital(
    ...
    match owners_data.get(&new_capital_id) {
        Some(&owner) if owner == country_id => {}
        Some(&owner) => return Err(format!(
    ...
    loop {
        if current_shell.is_empty() {
            return Err(format!(
                "BFS from original capital {} exhausted without finding any country {} territory",
                original_capital_id, country_id
            ));
        }
        ...
        if !eligible.is_empty() {
            // new_capital_id must be in this shell
            if eligible.contains(&new_capital_id) {
                return Ok(());
            } else {
                return Err(format!(
                    "New capital {} is not in the nearest eligible shell from original {}; \
                     nearest shell contains: {:?}",
                    new_capital_id, original_capital_id, eligible
                ));
            }
        }
...
pub fn perform_riot(
    ...
    if owns_data[&old_owner_id] == 0 {
        ...
        // Eliminated: capital resets to default (original), clear any override
        capitals.remove(&old_owner_id);
    }
}
...
pub fn perform_conquest(
    ...
    *owns_data.entry(original_conquered_country_id).or_insert(0) -= n_ceded;
    ...
    if owns_data[&original_conquered_country_id] == 0 {
        // Defender is eliminated — capital resets to default (original)
        remaining.remove(&original_conquered_country_id);
        capitals.remove(&original_conquered_country_id);
